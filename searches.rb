...
def IDA(new_game,fileName)
  outfile = File.open(fileName, "w")
  puts "-----------------Executing IDA search--------------------"
  ...
  threshold = new_game.get_f
  exceeded_thresholds = Array.new
  ...
  while !fringe.empty?
    ...
    if y_coord != 0 && board.get_history.last != "right"
      ...
      if state_copy.get_f > threshold
        # cut off this branch
        exceeded_thresholds << state_copy.get_f
      else
        fringe << state_copy
      end
    end
    ...
    # now set new threshhold for the next iteration
    # threshold = exceeded_thresholds.min
    threshold += 1
    # binding.pry
  end
